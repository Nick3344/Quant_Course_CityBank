...
class Pricer(ABC):
    ...
    def calc_delta(self, method: GreekMethod) -> float:
        if method != GreekMethod.BUMP:
            ...
            bump_model = copy.deepcopy(self.model)
            bump_model.bump_spot(bump_size)
            bump_pricer = self.create_pricer(self.contract,bump_model,Params())
            bumped_fv = bump_pricer.calc_fair_value()
        return bumped_fv  - self.calc_fair_value()
